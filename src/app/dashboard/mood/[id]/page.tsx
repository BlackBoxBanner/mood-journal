...
const MoodPageWithDetail = async ({
	params: { id },
}: {
	params: { id: string };
}) => {
			...
			<div className={"flex flex-col gap-8 lg:px-[12rem]"}>
				<div>
					<h2 className={cn("text-2xl")}>
						{format(new Date(mood.date), "EEEE, dd MMMM yyyy")}
					</h2>
				</div>
				<Card>
					<CardHeader>
					...
					<CardContent>{getMotto(mood.mood)}</CardContent>
				</Card>
				<Card>
					<CardHeader>
					...
					<CardContent className={"flex gap-2 flex-wrap"}>
						{mood.activities.split(" ").map((value, index) => {
							return (
								<span
									key={`${value}-${index}`}
									className={cn("bg-muted px-2 rounded")}
								>
									{value}
								</span>
							);
				...
				<Card>
					...
					<CardContent>{mood.grateful}</CardContent>
				</Card>
				<Card>
					<CardHeader>
						<CardTitle>{`What went well?`}</CardTitle>
					</CardHeader>
					<CardContent>{mood.well}</CardContent>
				</Card>
				<Card>
					<CardHeader>
						<CardTitle>{`What didn't work`}</CardTitle>
					</CardHeader>
					<CardContent>{mood.notWell}</CardContent>
				</Card>
			</div>
		</>
	);
