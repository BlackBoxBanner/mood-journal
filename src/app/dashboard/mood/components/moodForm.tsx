...
const MoodForm = ({callbackPath = "/dashboard/mood", userId, method = "create", id, ...mood}: MoodFormProps) => {
  ...
  const getCurrentDate = () => {
    ...
    if (date) {
      ...
      return isValid(formatDate) ? (isAfter(formatDate, new Date()) ? new Date() : formatDate) : new Date()
    }
    return new Date()
  }
              ...
              render={({field: {...field}}) => {
                return (
                  <FormItem className="col-span-4 lg:col-span-2">
                    ...
                    <FormControl>
                      <Textarea {...field} placeholder="I'm grateful for ..."/>
                    </FormControl>
                    <FormMessage/>
                  </FormItem>
                );
