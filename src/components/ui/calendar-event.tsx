  ...
  return new Array(7).fill(0).map((_, i) => {
    ...
    return (
      <ContextMenu key={`${currentDate}-${i}-${Math.random()}`}>
        <ContextMenuTrigger asChild>
          <button
            className={cn(
              ...
              isSameDay(currentDate, selectedDate) ? "bg-accent" : "",
          ...
          >
            <div className={cn("flex justify-end w-full")}>
              <p
                className={cn(
                  "w-full md:w-auto flex justify-center items-center px-2 ",
        ...
        <ContextMenuContent>
          {dayEvents.length !== 0 && options.map((option, index) => {
            return option.path ? (
              <>
                <Link href={dayEvents[0] && option.path ? option.path(dayEvents[0]) : "#"} passHref legacyBehavior
                      prefetch={false}>
                  <ContextMenuItem key={`ContextMenuItem-${currentDate}-${index}`}>
                    {option.label}
                  </ContextMenuItem>
                </Link>
              </>
              ...
              <>
                <ContextMenuItem
                  ...
                  onClick={() => option.onClick && option.onClick(dayEvents[0])}
                >
                  {option.label}
                </ContextMenuItem>
              </>
          ...
          {dayEvents.length === 0 && (
            <>
              <Link href={`/dashboard/mood/create?date=${new Date(currentDate.setHours(12)).toISOString()}`} passHref
                    legacyBehavior
                    prefetch={false}>
...
const Calendar = ({events = [], options}: CalendarProps) => {
